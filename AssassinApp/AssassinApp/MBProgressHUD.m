...
- (void) showCancelButtonWithTarget:(id)object action:(SEL)action {

//    UIButton * button = [[UIButton alloc] initWithFrame:CGRectMake(20, self.bounds.size.height - 60, self.bounds.size.width - 20, 40)];
    self.cancelButton = [[UIButton alloc] initWithFrame:self.bounds];
    [self.cancelButton addTarget:object action:action forControlEvents:UIControlEventTouchUpInside];
    [self setupCancelButton];
}

- (void) setupCancelButton {
    [self.cancelButton setTitle:@"Cancel" forState:UIControlStateNormal];
    self.cancelButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:13.0];
    [self.cancelButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    
    UIImage* image = [[UIImage imageNamed:@"modalViewBtnBG_iPhone"]
                      resizableImageWithCapInsets:UIEdgeInsetsMake(0, 5, 0, 6)];
...
- (void)layoutSubviews {
    ...
    CGSize cancelButtonSize = CGSizeMake(indicatorF.size.width*4, 30);

    
    NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:
                                label.font, NSFontAttributeName,
                                nil];
    CGSize labelSize = [label.text boundingRectWithSize:self.frame.size options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    ...
    if ( self.cancelButton ) {
       yPos = roundf(((bounds.size.height - totalSize.height) / 2)) + margin + yOffset - 15;
    }
    ...
    if ( self.cancelButton ) {
        ...
        if (cancelButtonSize.height > 0.f && (indicatorF.size.height > 0.f || detailsLabelSize.height > 0.f)) {
            yPos += 15;
        }
        cancelButtonFrame.origin.y = yPos;
        cancelButtonFrame.origin.x = roundf((bounds.size.width - cancelButtonSize.width) / 2) + xPos;
        cancelButtonFrame.size = cancelButtonSize;
...
- (void)unregisterFromNotifications {
    ...
    // this method is only called at dealloc. So removing self as observer from all notifs is fine.
